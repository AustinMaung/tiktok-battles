...
// 						// Select one winner randomly if multiple entries share max votes
// 						const randomIndex = Math.floor(Math.random() * rows.length);
// 						const winnerUrl = rows[randomIndex].url;
...
// 		} else {
// 			// Increment votes for the provided URL
// 			db.run('UPDATE videos SET votes = votes + 1 WHERE url = ?', [url], function(err) {
// 				if (err) {
// 					return res.status(500).json({ error: 'Database error' });
// 				}
// 				res.json({ message: 'Rating incremented successfully' });
...
// app.get('/get-random', (req, res) => {
// 	// SQL query to select a random entry
// 	const query = `SELECT url FROM videos ORDER BY RANDOM() LIMIT 1;`;
